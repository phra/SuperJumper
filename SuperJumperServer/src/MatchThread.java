...
public class MatchThread extends Thread implements PROTOCOL_CONSTANTS {
    ...
    public void start() {
        ...
        if (recv1.getState() == Thread.State.TERMINATED) {
            buffer.putPaccoOutNOBLOCK(new PaccoEnd());
            buffer.putPaccoInNOBLOCK(new PaccoEnd());
            System.out.println("recv1 terminated");
        } else if (recv2.getState() == Thread.State.TERMINATED) {
            buffer.putPaccoInNOBLOCK(new PaccoEnd());
            buffer.putPaccoOutNOBLOCK(new PaccoEnd());
            System.out.println("recv2 terminated");
        } else {
        ...
        this.close();
        System.out.println("TERMINO MATCHTHREAD");
        return;
    }
        ...
        public void run () {
            ...
            while(true){
                try {
                    ...
                    if (btsock1.writePkt(tmp) == -1) return;
                    //System.out.println("Send1Thread: writePkt()");
                    switch(tmp.getType()){
                        case PROTOCOL_CONSTANTS.PACKET_WELCOME:
                            System.out.println("Send1Thread: send pacco WELCOME.");
                            break;
                        case PROTOCOL_CONSTANTS.PACKET_START:
                            System.out.println("Send1Thread: send pacco START.");
                            break;
                        case PROTOCOL_CONSTANTS.PACKET_END:
                            System.out.println("Send1Thread: send pacco END.");
                            break;
    ...
    private class Recv1Thread extends Thread {
        ...
        public void run () {
            ...
            while(true){
                ...
                if (pkt == null) {
                    System.out.println("Recv1Thread: null");
                    latch.countDown();
                    return;
                }
                try {
                    ...
                    e.printStackTrace();
                    latch.countDown();
                    System.out.println("Recv1Thread: interrupted exception");
                    return;
                }
                if (pkt.getType() == PROTOCOL_CONSTANTS.PACKET_END){
                    latch.countDown();
                    System.out.println("Recv1Thread: end");
                    return;
                }
            }
        }
        ...
        public void run () {
            ...
            while(true){
                try {
                    ...
                    if (btsock2.writePkt(tmp) == -1) return;
